...
_.extend(RewriteRequest.prototype, {
  ...
  passApiKey: function(request, urlParts) {
    ...
    if(passApiKeyHeader) {
      ...
      request.headers[headerName] = request.apiUmbrellaGatekeeper.apiKey;
    } else {
    ...
    if(passApiKeyQueryParam) {
      if(urlParts.query[paramName] !== request.apiUmbrellaGatekeeper.apiKey) {
        urlParts.query[paramName] = request.apiUmbrellaGatekeeper.apiKey;
      ...
      if(urlParts.query[paramName]) {
        delete urlParts.query[paramName];
        urlParts.changed = true;
      }
    }
